    ...
    def tmm_bool_expr(self, bexpr: Expr, Lt, Lf) -> None:
        ...
        if isinstance(bexpr, Bool):
            ...
            if bexpr.op in {'EQUALITY', 'DISEQUALITY',
                ...
                self._emit('jmp', [Lf], None)
            elif bexpr.op in {'BOOLAND', 'BOOLOR'}:
                self.tmm_bool_expr(bexpr.args[0], Lt, Lf)
                self._emit('label', [self._fresh_label()], None)
...
#         if isinstance(stmt, ast.Assign) or isinstance(stmt, ast.Vardecl):
...
#     ap.add_argument('--continue', dest='keepgoing', action='store_true', default=False,
